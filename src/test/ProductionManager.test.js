...
test('renders ProductionManager component with HR and Budget sections', () => {
    render(
        <AuthProvider>
            <BrowserRouter>
                <ProductionManager 
                    requests={[{ id: 'REQ001', clientName: 'Client A', eventType: 'Conference' }]} 
    ...
    expect(screen.getByText(/Budget Request/i)).toBeInTheDocument();
});
...
test('submits a new HR request and clears form', () => {
    ...
    fireEvent.change(screen.getByLabelText(/Select Request ID:/i), { target: { value: 'REQ001' } });
    fireEvent.change(screen.getByLabelText(/Number of Staff Required:/i), { target: { value: '5' } });
    fireEvent.change(screen.getByLabelText(/Responsibilities:/i), { target: { value: 'Manage team operations' } });
    fireEvent.change(screen.getByLabelText(/Time Frame:/i), { target: { value: 'Q4 2023' } });
...
test('submits a new Budget request and clears form', () => {
    ...
    fireEvent.change(screen.getByLabelText(/Select Request ID:/i), { target: { value: 'REQ001' } });
    fireEvent.change(screen.getByLabelText(/Budget Amount:/i), { target: { value: '8000' } });
    fireEvent.change(screen.getByLabelText(/Reason for Budget:/i), { target: { value: 'Event coordination' } });

    // Submit Budget request form
    fireEvent.click(screen.getByText(/Submit Budget Request/i));

    // Verify that addBudgetRequest was called with the correct data
